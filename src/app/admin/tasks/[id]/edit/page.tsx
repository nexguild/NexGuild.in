...
export default function EditTaskPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setRequiredTaskIds(Array.isArray(data.required_task_ids) ? (data.required_task_ids as string[]) : []);
      setExcludedTaskIds(Array.isArray(data.excluded_task_ids) ? (data.excluded_task_ids as string[]) : []);
      setAllowsPartial(data.allows_partial_payment ?? false);
      setUnitName(data.unit_name ?? "");
      setTotalUnits(data.total_units != null ? String(data.total_units) : "");
      setPayPerUnitInr(data.pay_per_unit_inr != null ? String(data.pay_per_unit_inr) : "");
      setExtToolUrl(data.external_tool_url ?? "");
      setExtToolName(data.external_tool_name ?? "");
      setExtToolInstructions(data.external_tool_instructions ?? "");
      setExtProofType((data.external_proof_type as "screenshot" | "code" | "both") ?? "screenshot");
      setLoading(false);
    }
    load();
  }, [id]);
